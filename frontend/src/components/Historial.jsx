...
// Función para obtener clase CSS del badge según tipo de lesión
...
function Historial() {
    ...
    const applyFilters = (diagnosticos) => {
        ...
        if (searchValue) {
            if (searchType === 'id') {
                // Buscar por ID (usando el índice invertido)
                const total = diagnosticos.length;
                const indexMap = {};
                diagnosticos.forEach((d, idx) => {
                    indexMap[total - idx] = d.id;
                });
    ...
    const handleDelete = async (id, displayId) => {
    ...
    const handleViewResults = (diagnostico) => {
        if (typeof window.showDiagnosticoInResults === 'function') {
            window.showDiagnosticoInResults(diagnostico);
                                        ...
                                        <td style={{ textAlign: 'center' }}>
                                            <div className="actions-container" style={{ justifyContent: 'center' }}>
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => handleDelete(diagnostico.id, diagnostico.displayId)}
                                                >
